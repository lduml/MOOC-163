...
def parse_search(page):
    ...
    list_pageInfo = re.findall(re_pageInfo, page[-500:])  # 得到一个多维列表形式的匹配结果
...
# 交互
def user_interface(courseInfo, courseStat):
    ...
    while True:
        ...
        if re.match(r'^\d\d?\W*$', cmd):
            ...
            if 0 <= courseIndex <= curPageCount:
                try:
                    courseName = courseInfo[courseIndex][1].strip()
                    ...
                    print('>>>', courseName, '：', courseAbstract, '\n')
                    print('\t开课时间：%s\n\t结束时间：%s\n\t已参加人数：%s\n' % (startTime, endTime, enrollCount))
                except:
                    print('>>> 该课程暂未开通，请访问以下链接进行确认！')
                    print('>>> https://www.icourse163.org/search.htm?search={}#/'.format(quote(courseName)))
                    main()
...
def mass_dowmload(sourceList, sourceType):
    download_cnt = 0
    direction = download.select_direction()
    if sourceType is 1:  # 视频下载
        ...
        quality = None
        fileFormat = 'pdf'
    skipNum = input('>>> 是否继续下载？若是，请输入跳过文件数，否则请按Enter键：')
    ...
    for item in sourceList:
        ...
        print('\n[+]正在下载第{}份 - {}.{} ...\n'.format(download_cnt, name, fileFormat))
        if len(url) is 1:
...
def main():
    ...
    while True:
        ...
        download_type = user_interface(courseInfo, courseStat)
        if download_type:
            ...
            if sourceList:
                ...
                if re.match(r'^.*\n*$', cmd, re.I):
                    mass_dowmload(sourceList, download_type)
                else:
                    print('>>> 已退出下载！')
                break
